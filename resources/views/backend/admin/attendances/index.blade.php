  ...
  <div class="col-md-12">
    <div class="card ">
      <div class="card-header ">
        <h4 class="card-title">Input Absensi Pegawai</h4>
      </div>
      <div class="card-body ">
       @if (session('success'))
      ...
      <div class="text-center m-3">
        ...
        @else
        <a class="mr-2 font-weight-bold" href="{{ route('attendances.index', ['year' => $link['year'], 'month' => $link['month']]) }}">
          {{ $link['fullName'] }}
        </a>
        @endif
        @endforeach
      </div>
      <form action="{{ route('attendances.store', ['year' => $selectedYear, 'month' => $selectedMonth]) }}" method="post">
        @csrf
